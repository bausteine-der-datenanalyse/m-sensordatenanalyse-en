...
Solve for $\beta_1$:
$$
\beta_1 = \frac{\sum_{i=1}^{n} x_iy_i - n\bar{x}\bar{y}}{\sum_{i=1}^{n} x_i^2 - n \bar{x}^2} = \frac{\sum_{i=1}^{n} (x_i - \bar{x}) (y_i - \bar{y})}{\sum_{i=1}^{n}  (x_i - \bar{x})^2}
$$
...
Both `numpy.polynomial` (specifically `numpy.polynomial.polynomial`) and `scipy.stats.linregress` provide functions to compute this. Below we demonstrate the calculation with NumPy first, then show the SciPy functions. The operation of both modules is similar.
...
This is part of the old polynomial API. Since version 1.4, the new polynomial API defined in `numpy.polynomial` is recommended. A summary of the differences can be found in the [transition guide](https://numpy.org/doc/stable/reference/routines.polynomials.html).
